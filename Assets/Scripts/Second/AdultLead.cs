...
namespace MyGameApplication.Second
{
    public class AdultLead : MonoBehaviour
    {
        ...
        void OnTriggerEnter(Collider other)
        {
            ...
            {
                Dialogue.main.ShowDialogue("想过来？先让我看看你的能耐吧。", new Color(1.0f, 0, 0));
                m_Box = 1;
                m_Heal = 3;
            ...
            if (other.gameObject == m_LevelBeginGameObject[4])
            {
                if (m_Stage == 1)
                {
                    m_Scene.GetComponent<AdultScene>().m_LevelTime[4] = 0;
                    m_Scene.GetComponent<AdultScene>().m_LevelBegin[4] = true;
                    m_Stage++;
                }
                if(m_Stage==2 && m_Box==0 && m_Scene.GetComponent<AdultScene>().m_LevelBegin[4] == false)
                {
                    Dialogue.main.ShowDialogue("我喜欢什么？把你一路过来庇护你的东西交给我我再告诉你。", new Color(1.0f, 0, 0));
                }
                if (m_Stage == 2 && m_Box >= 1 && m_Scene.GetComponent<AdultScene>().m_LevelBegin[4] == false)
                {
                    m_Box=0;
